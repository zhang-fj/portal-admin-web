<template>
  <el-form ref="form" :inline="true" :model="formData" :rules="rules" size="small" label-width="80px" @submit.native.prevent>
    ...
    <el-form-item label="图片" prop="title">
      <image-upload v-if="formData.id" :key="formData.id" :tip="'建议图片尺寸为1920*420 '" :limit="1" :src-id="formData.id" :style="{width:'330px'}" :type="mainImageCate" @upload-success="uploadSuccessHanlder" @remove-success="removeSuccessHandler" />
    </el-form-item>
  </el-form>
</template>
...
export default {
  ...
  methods: {
    uploadSuccessHanlder(file) {
      if (file.id) {
        this.formData.mainImgId = file.id
      }
    },
    removeSuccessHandler(fileList) {
      if (fileList && fileList.length === 0) {
        this.formData.mainImgId = ''
      }
    }
  }
