<template>
  <div class="head-container">
    ...
    <curd-dialog :visible="dialog" :loading="submitBtnLoading" :title="title" @cancel-cu-handler="cancelCU" @submit-cu-handler="submitCU">
      <post-form ref="form" :data="formData" @submit-after-handler="submitAfterHandler" @submit-before-handler="submitBeforeHandler" @submit-error-handler="submitErrorHander" />
    </curd-dialog>
  </div>
</template>
...
import { page, del } from '@/api/bm02priv/post-api' // 导入【查询】和【删除】api
...
import postForm from './post-form' // 导入【表单】组件

export default {
  name: 'PostPage',
  components: { CurdOperation, Pagination, RrOperation, UdOperation, CurdDialog, postForm },
